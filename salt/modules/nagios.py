...
#Get nagios_config pillar to get plugin_dir, if not exist the pillar then assign a default path
PLUGINDIR = '/usr/lib/nagios/plugins'


def _set_plugindir():
    #Get nagios_config pillar to get plugin_dir, if not exist the pillar then assign a default path
    nagios_plugin_dir = __pillar__['nagios_config'][0]['plugin_dir']
    log.info('Pillar data {0}'.format(nagios_plugin_dir))
    if nagios_plugin_dir is None:
        plugindir = '/usr/lib/nagios/plugins/'
    else:
        plugindir = nagios_plugin_dir
    return plugindir
...
def _execute_cmd(plugin, args='', run_type='cmd.retcode'):
    ...
    log.info('PLUGINDIR {0}'.format(PLUGINDIR))
